...
/**
 * Default implementation of the {@link PersistenceUnitManager} interface. Used
 * as internal default by
 * {@link org.springframework.orm.jpa.LocalContainerEntityManagerFactoryBean}.
 * 
 * <p>
 * Supports standard JPA scanning for <code>persistence.xml</code> files, with
 * configurable file locations, JDBC DataSource lookup and load-time weaving.
 * 
 * <p>
 * The default XML file location is
 * <code>classpath*:META-INF/persistence.xml</code>, scanning for all matching
 * files in the class path (as defined in the JPA specification). DataSource
 * names are by default interpreted as JNDI names, and no load time weaving is
 * available (which requires weaving to be turned off in the persistence
 * provider).
 * 
 ...
 */
public class DefaultPersistenceUnitManager implements PersistenceUnitManager,
		ResourceLoaderAware, LoadTimeWeaverAware, InitializingBean {
	private static final Logger logger = LoggerFactory
			.getLogger(DefaultPersistenceUnitManager.class);
	/**
	...
	/**
	 * Default location for the persistence unit root URL: "classpath:",
	 * indicating the root of the class path.
	 */
	...
	private static final boolean jpa2ApiPresent = ClassUtils.hasMethod(
			PersistenceUnitInfo.class, "getSharedCacheMode");
	 ...
	 * These can be specified as Spring resource locations and/or location
	 * patterns.
	 * <p>
	 ...
	 *            an array of Spring resource Strings identifying the location
	 *            of the <code>persistence.xml</code> files to read
	 */
	...
	/**
	 * Set the default persistence unit root location, to be applied if no
	 * unit-specific persistence unit root could be determined.
	 * <p>
	...
	public void setDefaultPersistenceUnitRootLocation(
			String defaultPersistenceUnitRootLocation) {
		this.defaultPersistenceUnitRootLocation = defaultPersistenceUnitRootLocation;
	}

	/**
	 * Specify the name of the default persistence unit, if any. Default is
	 * "default".
	 * <p>
	 * Primarily applied to a scanned persistence unit without
	 * <code>persistence.xml</code>. Also applicable to selecting a default unit
	 * from several persistence units available.
	 * 
	...
	public void setPackagesToScan(String... packagesToScan) {
		logger.debug("PackagesToScan '{}'", packagesToScan);
	...
	/**
	 * Specify the JDBC DataSources that the JPA persistence provider is
	 * supposed to use for accessing the database, resolving data source names
	 * in <code>persistence.xml</code> against Spring-managed DataSources.
	 * <p>
	 * The specified Map needs to define data source names for specific
	 * DataSource objects, matching the data source names used in
	 * <code>persistence.xml</code>. If not specified, data source names will be
	 * resolved as JNDI names instead (as defined by standard JPA).
	 * 
	 ...
	 * <code>persistence.xml</code> against Spring-managed DataSource instances.
	 * <p>
	 * Default is JndiDataSourceLookup, which resolves DataSource names as JNDI
	 * names (as defined by standard JPA). Specify a BeanFactoryDataSourceLookup
	 * instance if you want DataSource names to be resolved against Spring bean
	 * names.
	 * <p>
	 * Alternatively, consider passing in a map from names to DataSource
	 * instances via the "dataSources" property. If the
	 * <code>persistence.xml</code> file does not define DataSource names at
	 ...
	 * <code>persistence.xml</code>.
	 * <p>
	 * In JPA speak, a DataSource passed in here will be uses as
	 * "nonJtaDataSource" on the PersistenceUnitInfo passed to the
	 * PersistenceProvider, provided that none has been registered before.
	 * 
	...
	/**
	 * Return the Spring LoadTimeWeaver to use for class instrumentation
	 * according to the JPA class transformer contract.
	 */
	...
	/**
	 * Prepare the PersistenceUnitInfos according to the configuration of this
	 * manager: scanning for <code>persistence.xml</code> files, parsing all
	 * matching files, configuring and post-processing them.
	 * <p>
	 * PersistenceUnitInfos cannot be obtained before this preparation method
	 * has been invoked.
	 * 
	 * @see #obtainDefaultPersistenceUnitInfo()
	...
	public void preparePersistenceUnitInfos() {
		...
		for (SpringPersistenceUnitInfo pui : puis) {
			...
			if (!this.persistenceUnitInfoNames.add(name)
				...
				msg.append(this.persistenceUnitInfos.get(name)
	...
	/**
	 * Read all persistence unit infos from <code>persistence.xml</code>, as
	 * defined in the JPA specification.
	 */
	private List<SpringPersistenceUnitInfo> readPersistenceUnitInfos() {
		PersistenceUnitReader reader = new PersistenceUnitReader(
				this.resourcePatternResolver, this.dataSourceLookup);
		List<SpringPersistenceUnitInfo> infos = new LinkedList<SpringPersistenceUnitInfo>();
		infos.addAll(Arrays.asList(reader
	...
	private SpringPersistenceUnitInfo buildDefaultPersistenceUnitInfo() {
		logger.debug("Building PersistentUnitInfo");
		SpringPersistenceUnitInfo scannedUnit = new SpringPersistenceUnitInfo();
		scannedUnit.setPersistenceUnitName(this.defaultPersistenceUnitName);
		scannedUnit.setExcludeUnlistedClasses(true);
		logger.debug("Packages To Scan '{}'", packagesToScan);
					...
					for (Resource resource : resources) {
						if (resource.isReadable()) {
							...
							if (matchesFilter(reader, readerFactory)) {
								logger.debug("Adding Managed Class '{}'",
										className);
								...
								if (scannedUnit.getPersistenceUnitRootUrl() == null) {
									...
									if (ResourceUtils.isJarURL(url)) {
										scannedUnit
												.setPersistenceUnitRootUrl(ResourceUtils
	 ...
	 * 
	 * @return the persistence unit root URL to pass to the JPA
	 *         PersistenceProvider
	...
	private URL determineDefaultPersistenceUnitRootUrl() {
		...
		try {
			Resource res = this.resourcePatternResolver
					.getResource(this.defaultPersistenceUnitRootLocation);
			return res.getURL();
		} catch (IOException ex) {
			throw new PersistenceException(
					"Unable to resolve persistence unit root URL", ex);
		}
	}

	/**
	 * Return the specified PersistenceUnitInfo from this manager's cache of
	 * processed persistence units, keeping it in the cache (i.e. not
	 ...
	 * implementations, detecting existing persistence units of the same name
	 * and potentially merging them.
	 * 
	 * @param persistenceUnitName
	 *            the name of the desired persistence unit
	 * @return the PersistenceUnitInfo in mutable form, or <code>null</code> if
	 *         not available
	 */
	protected final MutablePersistenceUnitInfo getPersistenceUnitInfo(
			String persistenceUnitName) {
		PersistenceUnitInfo pui = this.persistenceUnitInfos
				.get(persistenceUnitName);
		if (pui != null && Proxy.isProxyClass(pui.getClass())) {
			// JPA 2.0 PersistenceUnitInfo decorator with a
			// SpringPersistenceUnitInfo as target
			Jpa2PersistenceUnitInfoDecorator dec = (Jpa2PersistenceUnitInfoDecorator) Proxy
					.getInvocationHandler(pui);
	 ...
	 * PersistenceUnitPostProcessors. It is usually preferable to register
	 * further entity classes, jar files etc there rather than in a subclass of
	 * this manager, to be able to reuse the post-processors.
	 * 
	 * @param pui
	 *            the chosen PersistenceUnitInfo, as read from
	 *            <code>persistence.xml</code>. Passed in as
	 ...
	 * <p>
	 * Default is <code>false</code>. May be overridden to return
	 * <code>true</code>, for example if {@link #postProcessPersistenceUnitInfo}
	...
	public PersistenceUnitInfo obtainDefaultPersistenceUnitInfo() {
		if (this.persistenceUnitInfoNames.isEmpty()) {
			throw new IllegalStateException(
					"No persistence units parsed from "
							+ ObjectUtils
									.nullSafeToString(this.persistenceXmlLocations));
		}
		...
		PersistenceUnitInfo pui = this.persistenceUnitInfos.values().iterator()
				.next();
		this.persistenceUnitInfos.clear();
		return pui;
	}
	...
	public PersistenceUnitInfo obtainPersistenceUnitInfo(
			String persistenceUnitName) {
		PersistenceUnitInfo pui = this.persistenceUnitInfos
				.remove(persistenceUnitName);
		if (pui == null) {
			if (!this.persistenceUnitInfoNames.contains(persistenceUnitName)) {
				throw new IllegalArgumentException(
						"No persistence unit with name '" + persistenceUnitName
								+ "' found");
			} else {
				throw new IllegalStateException("Persistence unit with name '"
						+ persistenceUnitName + "' already obtained");
			}
		}
		return pui;
	}
	...
	/**
	 * Decorator that exposes a JPA 2.0 compliant PersistenceUnitInfo interface
	 * for a JPA 1.0 based SpringPersistenceUnitInfo object, adapting the
	 * <code>getSharedCacheMode</code> and <code>getValidationMode</code>
	 * methods from String names to enum return values.
	 */
	private static class Jpa2PersistenceUnitInfoDecorator implements
			InvocationHandler {
		...
		public Jpa2PersistenceUnitInfoDecorator(SpringPersistenceUnitInfo target) {
			this.target = target;
			try {
				this.sharedCacheModeEnum = (Class<? extends Enum>) ClassUtils
						.forName("javax.persistence.SharedCacheMode",
								PersistenceUnitInfo.class.getClassLoader());
				this.validationModeEnum = (Class<? extends Enum>) ClassUtils
						.forName("javax.persistence.ValidationMode",
								PersistenceUnitInfo.class.getClassLoader());
			} catch (Exception ex) {
				throw new IllegalStateException(
						"JPA 2.0 API enum types not present", ex);
			}
		}
